...
- **Visualizer** — build a view that overlays the *values* of registers picked from across devices, optionally alongside a synchronized camera video. Hit **＋ New Visualizer** in an experiment, click any register (or a single array position / CSV column) on any device card to add it — and/or a device's **🎞 video** row — then **Visualize**. Lines are plotted against the shared experiment time axis; each gets its own colour-matched Y-axis (or a shared one), values normalized per line by default. Scroll to zoom, left-drag to zoom-select, right-drag to pan; hover for the cursor time and every line's value there; click a legend entry to show/hide a line. Burst streams break across their silences instead of drawing a connecting line.
- **Camera video, synced to the data** — when a device folder contains video (`.avi`), GAPSCOPE plays it back **in the browser, with no conversion**, and draws a moving playhead line on the trace (or the Visualizer chart) at the matching time. Play/pause and a speed control sit above the view; a vertical line tracks the current video position, and **clicking the chart seeks the video** to that moment. Frame timing is taken from the camera's per-frame timestamp CSV, so the sync is exact across dropped frames and across the hourly clips a long recording is split into (seeking transparently swaps between clips). H.264-in-AVI is demuxed in JavaScript and decoded with the browser's [WebCodecs](https://developer.mozilla.org/docs/Web/API/WebCodecs_API) `VideoDecoder` — **Chrome or Edge required** (Firefox/Safari lack the API).
...
**Video** is handled the same self-contained way: the AVI container is parsed in JavaScript from header and index reads (frame byte offsets and keyframe flags come from the `idx1` index, so seeking never scans the whole file), individual frame chunks are read on demand, and the H.264 bitstream is decoded by the browser's WebCodecs `VideoDecoder`. A single position on the shared experiment clock drives both the on-screen frame and the trace playhead; seeking decodes forward from the nearest keyframe, holding the playhead at the target until that frame is ready so the decoder never chases a moving target.
