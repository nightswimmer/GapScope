...
- **Persisted settings** — thresholds (as multipliers of Δt), trace toggles, and the device schema library are saved to the browser's local storage and restored automatically on the next visit. Δt itself is never persisted — each stream re-detects its own.
...
4. Adjust **Nominal Δt**, **Gap threshold** (× Δt), and **Too-close under** (× Δt) in the header — changes apply immediately (no Apply button). **Load another capture** and, inside an experiment, **← devices** live in the header.
