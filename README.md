...
- **Combined value chart** — build a chart that overlays the *values* of registers picked from across devices. Hit **＋ New chart** in an experiment, click any register (or a single array position / CSV column) on any device card to add it, then **Visualize**. Lines are plotted against the shared experiment time axis; each gets its own colour-matched Y-axis (or a shared one), values normalized per line by default. Scroll to zoom, left-drag to zoom-select, right-drag to pan; hover for the cursor time and every line's value there; click a legend entry to show/hide a line. Burst streams break across their silences instead of drawing a connecting line.
